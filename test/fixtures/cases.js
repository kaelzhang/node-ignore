...
const cases = [
  ...
  [
    // A bracket expression never matches a slash, even when negated,
    //   so `a[!b]c` must not match `a/c` (matches git check-ignore / fnmatch(3))
    'negated class must not match slash: a[!b]c',
    [
      'a[!b]c'
    ],
    {
  ...
  [
    'negated class with caret must not match slash: x[^y]z',
    [
      'x[^y]z'
    ],
    {
